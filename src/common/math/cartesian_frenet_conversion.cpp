...
void CartesianFrenetConverter::cartesian_to_frenet(
  ...
  const double delta_theta = cyber_common::math::NormalizeAngle(theta - rtheta);
  const double tan_delta_theta = std::tan(delta_theta);
  const double cos_delta_theta = std::cos(delta_theta);

  const double one_minus_kappa_r_d = 1 - rkappa * ptr_d_condition->at(0);
  if((fabs(delta_theta) < 3*M_PI/4) && (fabs(delta_theta) > M_PI/4)) {
    ptr_d_condition->at(1) = cos(M_PI/2 + rtheta - theta);
  } else {
...
void CartesianFrenetConverter::frenet_to_cartesian(
  ...
  *ptr_x = rx - sin_theta_r * d_condition[0];   //d向左为正？
  ...
  *ptr_theta = cyber_common::math::NormalizeAngle(delta_theta + rtheta);
...
double CartesianFrenetConverter::CalculateTheta(const double rtheta,
                                                ...
                                                const double l,
                                                const double dl) {
  return cyber_common::math::NormalizeAngle(rtheta + std::atan2(dl, 1 - l * rkappa));
}
